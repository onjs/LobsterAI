...
import type { ScheduledTaskBackendService } from '../scheduledTask/backendService';
...
import {
  getCronJobService as getCronJobServiceFromManager,
...
const getCronJobService = (): CronJobService => getCronJobServiceFromManager();
...
const stopScheduledTaskPollingForBackend = (backend: Exclude<STBackendType, 'auto'>): void => {
  ...
  try {
    ...
    // Cron job service may not be initialized yet.
  }
};
    ...
    getIMGatewayManager: () => ({
      getIMStore: () => ({
        getSessionMapping: (conversationId: string, platform: string) =>
          getIMGatewayManager().getIMStore().getSessionMapping(conversationId, platform as IMPlatform),
        listSessionMappings: (platform: string, accountId?: string) =>
          getIMGatewayManager().getIMStore().listSessionMappings(platform as IMPlatform, accountId),
      }),
      primeConversationReplyRoute: (
        platform: string,
        conversationId: string,
        coworkSessionId: string,
      ) => getIMGatewayManager().primeConversationReplyRoute(platform as IMPlatform, conversationId, coworkSessionId),
    }),
    getOpenClawRuntimeAdapter: () => openClawRuntimeAdapter,
  });
